...
export default function DataSheetPage() {
  ...
  return (
    <div className="w-full min-h-screen bg-[#ffffff]">
      {/* Hero Section */}
      <section className="relative w-full h-[500px] max-[1454px]:h-auto mx-auto flex flex-col justify-center items-center overflow-hidden bg-[url('/images/pages/resources/DataSheet/hero-bg.svg')] bg-cover bg-center bg-no-repeat bg-[#030D1A] opacity-100 p-[120px_20px_80px] max-[1454px]:p-[160px_20px_100px] text-center">
        <div className="relative w-full max-w-[850px] flex flex-col items-center gap-[24px] text-center z-[5] px-[20px] mx-auto">
          <div className="flex justify-center items-center w-auto h-[38px] gap-[8.12px] rounded-[27.61px] p-[8.12px_24px] bg-[#FFFFFF12] border-[0.81px] border-[#FFFFFF26] m-0">
            <span className="opacity-100 font-manrope font-semibold text-[14px] leading-none tracking-[0.09em] uppercase text-[#FFFFFF] bg-transparent border-none whitespace-nowrap">DATA DEEP DIVE</span>
          </div>
          ...
          <div className="grid grid-cols-3 gap-[30px] max-[1024px]:grid-cols-2 max-[768px]:grid-cols-1">
            {filteredDatasheets.length > 0 ? (
              filteredDatasheets.map((item) => (
                <article key={item.id} className="bg-white border border-[#E5E7EB] rounded-[16px] p-[40px] flex flex-col h-full transition-all duration-300 hover:shadow-xl group">
                  {/* Icon */}
                  ...
                  {/* Content */}
                  <h3 className="text-[24px] font-bold text-[#030D1A] mb-[16px] font-sora">
                    {item.title}
                  </h3>
                  <p className="text-[15px] leading-[26px] text-[#4A5568] mb-[24px] font-manrope flex-grow">
                    {item.description}
                  </p>
